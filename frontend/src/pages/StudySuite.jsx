...
const StudySuite = () => {
    ...
    const renderQuiz = () => {
        ...
        return (
            <div className="bg-white rounded-md shadow-lg border border-slate-100 p-6 max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                    {questions.map((q, qIndex) => (
                        <div key={qIndex} className="p-4 bg-slate-50 rounded-md">
                                ...
                                {q.options.map((opt, oIndex) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-3 text-left rounded-md border-2 transition-all ${isCorrect ? 'bg-green-100 border-green-500 text-green-800' : isWrong ? 'bg-red-100 border-red-500 text-red-800' : isSelected ? 'bg-primary/10 border-primary' : 'bg-white border-slate-200 hover:border-primary'}`}
                                        >
                                            {opt}
                                        </button>
                                    );
                ...
                <div className="mt-6 flex justify-center gap-4">
                    {!quizSubmitted ? (
                        <button onClick={() => setQuizSubmitted(true)} disabled={Object.keys(quizAnswers).length < questions.length} className="px-8 py-3 bg-primary text-white font-bold rounded-md hover:bg-primary-dark transition-all disabled:opacity-50">
                            Submit Quiz
                        </button>
                    ) : (
                        <button onClick={() => { setQuizSubmitted(false); setQuizAnswers({}); }} className="flex items-center gap-2 px-8 py-3 bg-slate-100 text-slate-700 font-bold rounded-md hover:bg-slate-200 transition-all">
                            <Undo /> Retake
                        </button>
                    )}
                </div>
    ...
    const renderFlashcards = () => {
        ...
        return (
            <div className="bg-white rounded-md shadow-lg border border-slate-100 p-6 flex flex-col items-center h-[400px]">
                <div className="flex justify-between items-center w-full mb-4">
                    <h2 className="text-xl font-bold text-slate-800">Flashcards</h2>
                    <button onClick={() => setGeneratedContent(null)} className="p-2 text-slate-400 hover:text-red-500"><X /></button>
                </div>
                <div onClick={() => setIsFlipped(!isFlipped)} className="flex-grow w-full flex items-center justify-center cursor-pointer bg-gradient-to-br from-primary/5 to-secondary/10 rounded-md border-2 border-dashed border-slate-200 p-6">
                ...
                <div className="flex items-center gap-6">
                    <button onClick={() => { setFlashcardIndex(i => Math.max(0, i - 1)); setIsFlipped(false); }} disabled={flashcardIndex === 0} className="p-3 bg-slate-100 rounded-md hover:bg-slate-200 disabled:opacity-30"><ArrowLeft /></button>
                    <span className="font-bold text-slate-600">{flashcardIndex + 1} / {cards.length}</span>
                    <button onClick={() => { setFlashcardIndex(i => Math.min(cards.length - 1, i + 1)); setIsFlipped(false); }} disabled={flashcardIndex === cards.length - 1} className="p-3 bg-slate-100 rounded-md hover:bg-slate-200 disabled:opacity-30"><ArrowRight /></button>
                </div>
            </div>
        );
    };
            ...
            <div className="flex-grow flex flex-col lg:flex-row gap-6 p-6 pt-24">
                ...
                >
                    {isDragging && (
                        <div className="absolute inset-0 flex items-center justify-center bg-white/80 z-20 backdrop-blur-sm">
                            <div className="text-center p-8 border-4 border-dashed border-primary rounded-md animate-pulse">
                    ...
                    <div className="bg-gradient-to-r from-primary to-secondary p-4 text-white flex items-center justify-between">
                        ...
                        <label className={`flex items-center gap-2 px-4 py-2 bg-white/20 text-white rounded-md cursor-pointer hover:bg-white/30 transition-all ${isUploading ? 'opacity-50' : ''}`}>
                    ...
                    {selectedDoc && (
                        <div className="p-4 border-t border-slate-100 bg-white flex gap-4 justify-center">
                            <button onClick={() => handleGenerate('quiz')} disabled={isGenerating} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-400 to-orange-500 text-white font-bold rounded-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-wait">
                                {isGenerating && genType === 'quiz' ? <Loader2 className="animate-spin" /> : <List />}
                                Generate Quiz
                            </button>
                            <button onClick={() => handleGenerate('flashcards')} disabled={isGenerating} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-teal-400 to-cyan-500 text-white font-bold rounded-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-wait">
                ...
                <div className="lg:w-1/2 flex flex-col">
                    {generatedContent ? (
                        ...
                        <div className="flex-grow bg-white rounded-md shadow-lg border border-slate-100 overflow-hidden flex flex-col">
                            {/* Chat Header */}
                            ...
                            <div className="flex-grow p-4 overflow-y-auto space-y-4 bg-slate-50">
                                ...
                                {messages.map((msg, i) => (
                                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`p-4 rounded-md max-w-[85%] shadow-sm ${msg.role === 'user' ? 'bg-secondary text-white rounded-tr-none' : 'bg-white border border-slate-100 text-slate-700 rounded-tl-none'}`}>
                                            <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                                        </div>
                                    </div>
                                ))}
                                {isSending && <div className="flex justify-start"><div className="p-4 bg-white border border-slate-100 rounded-md rounded-tl-none shadow-sm"><Loader2 className="animate-spin text-primary" /></div></div>}
                                <div ref={chatEndRef} />
                            </div>
                            {/* Chat Input */}
                            <div className="p-4 bg-white border-t border-slate-100">
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-grow p-3 bg-slate-50 rounded-md border border-slate-200 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary disabled:opacity-50"
                                        value={inputMessage}
                                        onChange={(e) => setInputMessage(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && sendMessage()}
                                        disabled={!selectedDoc || isSending}
                                    />
                                    <button onClick={sendMessage} disabled={!selectedDoc || isSending || !inputMessage.trim()} className="p-3 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors disabled:opacity-50">
                                        <Send />
                                    </button>
                                </div>
                            </div>
