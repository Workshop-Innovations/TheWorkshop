...
import { FaClock, FaListAlt, FaChartBar, FaStore, FaCoins, FaTasks, FaBrain, FaUserCircle, FaUsers, FaMagic } from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    const [selectedTitle, setSelectedTitle] = useState(() => {
        ...
        return savedTitle ? JSON.parse(savedTitle) : (REWARDS?.PASSIVE_TITLES?.find(r => r.id === 'title-novice'));
    });
    const [selectedFrame, setSelectedFrame] = useState(() => {
        const savedFrame = localStorage.getItem('userFrame');
        return savedFrame ? JSON.parse(savedFrame) : (REWARDS?.PASSIVE_FRAMES?.find(r => r.id === 'frame-basic'));
    });

    // --- REWARD LOGIC ---
    const isEarned = (reward) => {
        const { minutesRequired } = reward;
        // Use totalFocusMinutes for unlocking
        return (userStats?.totalFocusMinutes || 0) >= minutesRequired;
    };
    ...
    const totalFocusTime = totalFocusMinutes * 60;
            ...
            <main className="flex-grow container mx-auto px-4 pt-24 pb-16">
                ...
                <header className="mb-12 flex flex-wrap justify-between items-start gap-6">
                    <div className="flex items-center gap-6 p-4 bg-[#1A1A1A] rounded-xl shadow-lg border border-gray-700/50">
                        {/* Profile Picture Display with Image Frame */}
                        <div className={`relative w-33 h-33 flex items-center justify-center`}>
                            ...
                            {/* Inner Profile Picture Container Layer */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pb-10">
                    ...
                    <FeatureCard to="/community" icon={<FaUsers />} title="Community" description="Chat with other users" />
                    <FeatureCard to="/flashcards" icon={<FaMagic />} title="Flashcards" description="AI-generated study sets" />
                </div>
                ...
                {isSettingsOpen && (
                    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            {/* Title Selector */}
                            <div className="mb-8 border-b border-gray-700 pb-6">
                                <h4 className="text-xl font-semibold mb-3 text-gray-300">Select Title (Earned: {totalFocusMinutes} Minutes)</h4>
                                        ...
                                        availableTitles.map(title => (
                                            ...
                                            >
                                                ...
                                                <p className="text-xs text-gray-500">
                                                    Requires: {title.minutesRequired} Minutes
                                                </p>
                                            </button>
                                        ))
                                    ) : (
                            ...
                            {/* Frame Selector */}
                            <div className="mb-8">
                                <h4 className="text-xl font-semibold mb-3 text-gray-300">Select Frame (Earned: {totalFocusMinutes} Minutes)</h4>
                                <div className="grid grid-cols-2 gap-4 max-h-48 overflow-y-auto pr-2">
                                        ...
                                        availableFrames.map(frame => (
                                            ...
                                            >
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">
                                                    Requires: {frame.minutesRequired} Minutes
                                                </p>
                                            </button>
                                        ))
                                    ) : (
